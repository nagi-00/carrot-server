...
// 카카오 썸네일은 R120x174 같은 크기 토큰이 URL에 박혀 있습니다.
// 이를 키워 더 선명한 표지를 받습니다. (비공식 동작이므로 실패해도 원본을 그대로 씁니다)
function upscaleKakaoCover(url) {
  if (!url) return '';
  return url.replace(/\/thumb\/R\d+x\d+/, '/thumb/R320x464');
}
...
export async function searchBooks(q, k) {
  ...
  try {
    ...
    const needsHelp = kakao.length === 0 || kakao.some((b) => !b.cover);

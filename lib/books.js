...
// 카카오 썸네일은 R120x174 하나만 서빙하며(다른 크기는 응답하지 않음), 영수증 PNG에는 너무 작습니다.
// URL의 fname에 담긴 원본 주소를 꺼내 씁니다. 단, 표지 이미지로 신뢰할 수 있는
// 다음 책 CDN(t1.daumcdn.net/lbook)일 때만 사용하고, 그 외에는 썸네일을 유지합니다.
function upscaleKakaoCover(url) {
  if (!url) return '';
  try {
    const m = String(url).match(/[?&]fname=([^&]+)/);
    if (m) {
      let original = decodeURIComponent(m[1]);
      if (/^https?:\/\/[^/]*daumcdn\.net\/lbook\//.test(original)) {
        original = original.split('?')[0];        // timestamp 등 부가 파라미터 제거
        return original.replace(/^http:\/\//, 'https://');  // 혼합 콘텐츠 방지
      }
    }

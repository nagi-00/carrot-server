...
// 카카오 썸네일 경로(/thumb/R{w}x{h}.q{품질})는 동적 리사이저입니다.
// 기본 R120x174는 영수증 PNG(2배 배율)에 부족하므로 크기와 품질을 함께 올립니다.
function upscaleKakaoCover(url) {
  if (!url) return '';
  return String(url).replace(/\/thumb\/R\d+x\d+(\.q\d+)?/, '/thumb/R360x522.q95');
}
